...
function enterWorld(tracker: Tracker, nonce: string | null, name: string, gm: boolean) {
  ...
  session.feedServer(
    wire.server(characterList([{ name, cls: 4, level: 50, status: gm ? 32 : 0 }]))
  );
  session.feedClient(wire.client(selectCharacter(name)));
  session.feedServer(
    wire.server(characterInformation({ x: 100, y: 100, map: 0, status: gm ? 32 : 0 }))
  );
  return { session, wire };
}
...
describe('AdminHub', () => {
  ...
  it('sends the snapshot on attach and the deltas after', () => {
    ...
    expect(socket.sent.find(m => m.t === 'leave')).toEqual({
      t: 'leave',
      id: session.id,
    });
  ...
  it('answers a log query from the journal, newest first, with paging', () => {
    ...
    for (let i = 0; i < 5; i++) {
      session.feedClient(wire.client(chat('Aeris', `line ${i}`)));
    }
    ...
    if (!answer || answer.t !== 'log') {
      throw new Error('expected a log answer');
    }
    ...
    expect(
      answer.events.map((e: TrackEvent) => e.text)
    ).toEqual(['line 4', 'line 3', 'line 2']);
    ...
    hub.receive(
      ...
      JSON.stringify({
        ...
        kinds: ['chat'],
        limit: 3,
        before: answer.events[2].at,
      })
    );

    const older = socket.sent.filter(m => m.t === 'log')[1];

    if (!older || older.t !== 'log') {
      throw new Error('expected a second page');
    }

    // All test events share the same timestamp, so `before` excludes them all.
