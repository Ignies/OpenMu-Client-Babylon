...
class Counter {
  ...
  /**
   * Wraps 255 -> 0 like OpenMU's `Counter` (Network/Counter.cs, `_maxCount`
   * = 255): the value travels as one byte in the first decrypted block, so an
   * unbounded count would disagree with the wire from the 257th C3/C4 packet
   * on and every later packet would be rejected.
   */
  Increase() {
    this.Count = this.Count === 255 ? 0 : this.Count + 1;
  }
}
...
export class SimpleModulusDecryptor extends SimpleModulusBase {
  ...
  /**
   * Reject blocks whose checksum does not match, as OpenMU's decryptor does by
   * default (`AcceptWrongBlockChecksum` is false there too): with the size
   * check alone ~1% of random garbage still passes, with the checksum none does.
   * Set to true to only take the size check (`lastError` then stays null).
   */
  public AcceptWrongBlockChecksum = false;

  /**
   * Why the last `Decrypt` call failed (or "counter ..." when it resynced),
   * for the caller's log line. Null when the last packet was clean.
   */
  public lastError: string | null = null;

  /**
   * Decrypts a C3/C4 packet and returns it with its own (smaller) length
   * header; C1/C2 pass through. The caller keeps advancing its byte queue by
   * the *wire* length - only the returned packet is the decrypted size.
   *
   * The first decrypted byte is a per-packet counter that must match ours.
   * OpenMU disconnects on a mismatch (InvalidPacketCounterException); a client
   * cannot afford that, so when the blocks themselves decode cleanly we
   * *resync* to the received counter and accept the packet - one lost or
   * duplicated packet then costs nothing, and `lastError` says "counter" so
   * the caller can log it. Only a packet whose blocks are malformed (content
   * not a multiple of 11 bytes, block size > 8, bad checksum when rejected)
  ...
  Decrypt(packet: Uint8Array): [boolean, Uint8Array] {
    ...
    if (r == null) {
      // Malformed blocks: the counter byte is meaningless, so leave ours
      // untouched - the next well-formed packet resyncs it if needed.
      return [false, packet];
    }

    const [received, decrypted] = r;

    if (this.Counter.Count !== received) {
      this.lastError = `counter (expected ${this.Counter.Count}, got ${received})`;
      this.Counter.Count = received;
    }
  ...
  private DecryptC3(data: Uint8Array): [Byte, Uint8Array] | null {
    const contentSize = this.GetContentSize(data, false);
    const headerSize = getPacketHeaderSize(data);

    if (contentSize <= 0 || (contentSize % EncryptedBlockSize) !== 0) {
      this.lastError = `content size ${contentSize} is not a multiple of ${EncryptedBlockSize}`;
      return null;
    }
  ...
  /** Returns the decrypted content size, or -1 when a block is malformed. */
  ...
  private DecodeFinal(inputBuffer: Uint8Array, outputBuffer: Uint8Array): Int {
    ...
    if (blockSize > DecryptedBlockSize) {
      // Garbage block (wrong key or corrupted stream). Copying `blockSize`
      // bytes out of the 8-byte CryptBuffer would throw a RangeError, which
      // used to escape the socket's packet loop and jam it for good.
      this.lastError = `block size ${blockSize} > ${DecryptedBlockSize}`;
      return -1;
    }

    // The checksum covers the whole 8-byte block (zero padded), as in
    // OpenMU's PipelinedSimpleModulusDecryptor.DecodeFinal.
    BufferBlockCopy(this.CryptBuffer, 0, outputBuffer, 0, DecryptedBlockSize);
    let checksum: Byte = BlockCheckSumXorKey;
    for (let i = 0; i < DecryptedBlockSize; i++) {
      checksum ^= outputBuffer[i];
    }

    if (this.ShiftBuffer[1] !== checksum && !this.AcceptWrongBlockChecksum) {
      this.lastError = `checksum (expected ${checksum}, got ${this.ShiftBuffer[1]})`;
      return -1;
    }
