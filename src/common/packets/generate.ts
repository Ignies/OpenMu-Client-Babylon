...
/**
 * OpenMU leaves `ItemData` unsized inside `StoredItem` / `DroppedItem` style
 * structures because the byte count belongs to the server's item serializer,
 * not the packet. Season 6 serialises an item in 12 bytes
 * (`ItemSerializer.NeededSpace`). Without this, the reader emitted a
 * zero-length slice and a 1-byte stride, so every item in `CharacterInventory`
 * (and `ItemsDropped`) decoded from an empty buffer.
 */
const UNSIZED_ITEM_DATA_LENGTH = 12;

function structureFieldLength(field: { type: PacketFieldType; name: string; length?: number }): number | undefined {
  if (field.length != null) return field.length;
  if (field.type === 'Binary' && field.name === 'ItemData') return UNSIZED_ITEM_DATA_LENGTH;
  return undefined;
}
...
function getSizeOfStructure(p: Packet, s: Structure): string | number {
  ...
  if (lastField) {
    if (lastField.type.endsWith('[]')) {
      const prevSize = prevField.index + getTypeSizeInBytes(prevField.type, structureFieldLength(prevField));
    ...
    return lastField.index + getTypeSizeInBytes(lastField.type, structureFieldLength(lastField));
...
${s.fields.map(({ type, name, typeName, length, itemCountField, index, leftShifted }, fieldIndex, fields) => {
    ...
    const fieldLength = structureFieldLength({ type, name, length });
    const bytesInField = getTypeSizeInBytes(type, fieldLength);

    return `      const ${name} = ${getReaderStringForType(type, `${fieldName}_StartOffset + ${index}`, 'b', leftShifted ?? 0, bytesInField, fieldLength)};`;
  }).join('\n')}
      ${fieldName}[i] = {
${s.fields.map(({ name, }) => {
    return `        ${name}`;
...
function generateStructureDefinition(p: Packet, s: Structure): string {
  ...
  s.fields.forEach(({ type, name, typeName }) => {
    ...
    if (type === 'Enum') {
      tsType = getEnumName(p, typeName ?? type);
    } else if (type === 'Structure[]') {
      const nested = p.structures.find(s => s.name === typeName) ?? globalStructures.find(s => s.name === typeName);
      if (!nested) {
        const e = new Error('no struct ' + typeName);
        console.error(e);
        throw e;
      }
      tsType = generateStructureDefinition(p, nested) + '[]';
    } else if (type === 'String') {
      tsType = 'string';
    }
    structureFields += `  ${name}: ${tsType};\n`;
  });
