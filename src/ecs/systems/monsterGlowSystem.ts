 ...
 * glow - and their eyes - once the model is posed, drops it when they die
 * or leave. Owns no visual state - `effects/monsterGlow.ts` does.
 *
...
export const MonsterGlowSystem: ISystemFactory = world => {
  ...
  function snuff(e: Entity): void {
    for (const h of glows.get(e) ?? []) h.stop();
    glows.delete(e);
  }
  ...
  return {
    update: () => {
      for (const e of characters) {
        const parts = monsterGlowFor(e.npcType);
        if (!parts) continue;
        ...
        if (glows.get(e)?.some(h => h.alive)) continue;
        ...
        glows.set(
          e,
          parts.map(glow => glowMonster(world.scene, e, glow))
        );
      }
    },
  };
